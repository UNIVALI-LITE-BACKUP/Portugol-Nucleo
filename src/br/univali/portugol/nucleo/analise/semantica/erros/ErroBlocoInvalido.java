...
public final class ErroBlocoInvalido extends ErroSemantico
{
    ...
    {
        return "Esta expressão não faz sentido se estiver sozinha no código. Você deve atribuir a expressão a uma variável, vetor ou matriz";
    }    
    ...
    protected String construirMensagem()
    {
        ExpressionTextExtractor extractor = new ExpressionTextExtractor();
        String expressionText = extractor.extract(bloco);
        
        return String.format("A expressão '%s' não faz sentido se estiver sozinha no código. Você deve atribuir esta expressão a uma variável, vetor ou matriz", expressionText);
    }
    
    
    private final class ExpressionTextExtractor implements VisitanteASA
    {
        public String extract(NoBloco bloco)
        {
            try
            {
                return (String) bloco.aceitar(ExpressionTextExtractor.this);
            }
